    ...
    <div class="data-table table-striped">
        <table class="">
            ...
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo (get_member($vo["level"])); ?></td>
                        <td><?php echo ($vo["username"]); ?></td>
                        <td><?php echo ($vo["phone"]); ?></td>
                        <td><?php echo ($vo["company"]); ?></td>
                        <td><?php echo (get_dep($vo["position"])); ?></td>
                        <td><span><?php echo (time_format($vo["addtime"])); ?></span></td>
                        <td>
                            <?php switch($vo["is_pass"]): case "1": ?>已通过<?php break;?>
                                <?php case "-1": ?>未通过<?php break;?>
                                <?php case "0": ?>审核中<?php break;?>
                                <default>
                                    -
                                </default><?php endswitch;?>
                        </td>
                        <td>
                                <a href="<?php echo U('User/info?uid='.$vo['id']);?>" class="authorize">查看</a>
                                <a href="<?php echo U('User/memberChecked?type=del&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                        </td>
