...
<script>
	...
	<div class="cf">
		...
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入讲座标题和发布人">
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>已试听<?php echo ($vo["number"]); ?>/次</td>
			<td><?php echo ($vo["username"]); ?></td>
			<td><?php echo (date('Y-m-d H:i',$vo["adtime"])); ?></td>
			<td><?php if(($vo['is_tj']) == "1"): ?>是<?php else: ?>否<?php endif; ?></td>
			<td>
				<?php if(($vo['is_tj']) == "1"): ?><a href="<?php echo U('Lecture/changeStatus?status=0&id='.$vo['id']);?>" class="confirm ajax-get">取消</a>
                <?php else: ?>
				<a href="<?php echo U('Lecture/changeStatus?status=1&id='.$vo['id']);?>" class="confirm ajax-get">推荐</a><?php endif; ?>
                <a href="<?php echo U('info?cate_id='.$_GET['cate_id'].'&id='.$vo['id']);?>">详情</a>
            </td>
	...
	<script src="/qiyun/Public/static/thinkbox/jquery.thinkbox.js"></script>
